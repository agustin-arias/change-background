...
downloadImage(){
    ...
    image=$(curl -s $images | xmllint --html --xpath "string($xpathImage)" - 2>/dev/null)
    ...
    downloadWallpaper=$(curl -s $imagePage | xmllint --html --xpath "string($xpathDownload)" - 2>/dev/null)
    ...
    fullImage=$(curl -s $downloadWallpaperPage | xmllint --html --xpath "string($xpathFullImage)" - 2>/dev/null)
